...
{
    for (int x = destCenterX - destRadius; x < destCenterX + destRadius; x++)
    {
        for (int y = destCenterY - destRadius; y < destCenterY + destRadius; y++)
        {
            if (pow (x - destCenterX, 2) + pow (y - destCenterY, 2) <= pow (destRadius, 2))
            {
                int sourceX = sourceCenterX + (x - destCenterX);
                int sourceY = sourceCenterY + (y - destCenterY);
...
int SDLWindow::RenderDrawCircle (SDL_Renderer* renderer, int x, int y, int radius)
{
