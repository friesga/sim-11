#ifndef _ZIPFILE_READER_H_
#define _ZIPFILE_READER_H_

//
// A aimple zipfile reader. Allows extraction of files in a mapped zipfile.
// 
// Based upon // https://github.com/andy-thomason/andyzip/tree/master
//
// (C) Andy Thomason 2016. MIT-license
//
// Changes:
// - Changed class and public function names to Pascal style
//
...
{
public:
    ZipFileReader (const uint8_t* begin, const uint8_t* end)
        :
        begin_ (begin),
        end_ (end)
    {
    ...
    std::vector<const uint8_t*> dirEntries () const
    {
        // central file header signature   4 bytes  (0x02014b50)
        // version made by                 2 bytes (+4)
        // version needed to extract       2 bytes (+6)
    ...
    {
        const uint8_t* p = getDirEntry (filename);
    ...
    std::vector<uint8_t> readEntry (const uint8_t* p) const
    {
